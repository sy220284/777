...
transition = _load("v46_min_transition", ROOT / "tools" / "v4_6_transition_features.py")
v4 = transition.v4
v45 = transition.v45


def min_rank_ensemble(raw: dict[str, np.ndarray]) -> np.ndarray:
    ...
    for t in range(n):
        ...
        for name in names:
            ...
            if not np.isfinite(p).all():
                break
            ranks.append(v4._rank_score(p))
        if len(ranks) == len(names):
            out[t] = np.min(np.stack(ranks), axis=0)
    return out
...
def main() -> None:
    ...
    robust = min_rank_ensemble(raw)
    blue_base, _ = v4.walk_forward_blue(df)
    blue_models = v45.build_blue_models(df)

    report = {
        "experiment": "V4.6三时间尺度最差排名共识",
        "rule": {
            "models": ["全历史", "最近1000期", "最近500期"],
            "aggregation": "逐号取三个排名分位中的最低值",
            "meaning": "号码必须在所有时间尺度都维持较高排名才可进入前列",
            "parameter_search": False,
        },
        "configs": {},
        "promotion": {},
    }
    for name, pred in (("base", base), ("rank_min", robust)):
    ...
    out = ROOT / "backtests" / "v4_6" / "rank_min_ensemble.json"
