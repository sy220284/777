...
class PersonaAutoFillService @Inject constructor(
    ...
    private companion object {
        ...
        val SYSTEM_PROMPT = """
            ...
            3. personality 和 speechStyle 要写成可执行的行为规则，避免空泛形容词。
            4. relationship 要明确角色如何看待用户、亲疏边界、称呼习惯和互动倾向。
            5. hardConstraints 写 4-10 条真正会影响扮演稳定性的硬规则。
            6. exampleDialogues 写 3-6 条短对白，体现语气、节奏和潜台词，不写长段剧情。
            7. bannedPhrases 只放会明显破坏角色感或产生机械 AI 味的表达。
            8. signaturePhrases 放少量自然常用表达，避免每句话都重复口头禅。
            9. 数组字段必须是 JSON 字符串数组；没有合适内容时返回空数组。
            10. 输出必须是可直接解析的标准 JSON。
        """.trimIndent()
    }
}
