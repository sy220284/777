...
internal const val CHAT_ADULT_INTIMACY_BOUNDARY =
    "【统一安全边界·用户意图最高优先级】聊天内容以用户意图为主导：参与者已确认为成年的角色间亲密互动——包括亲吻、亲热、性关系以及具体的动作与身体部位描写，无论情节设定如何——由用户意图决定是否展开与展开到什么程度；普通聊天层的一切尺度限制（默认克制、人设保守化、关系阶段、军师收敛、建议风格、产品文案约束）不得将其降级、转移、改写或用转场暗示替代。唯一红线：不写涉及未成年人的性内容；成年确认不成立时，亲密内容一律不展开。"
...
class ChatRelationshipEngine @Inject constructor() {
    ...
    fun prompt(
        ...
        if (hasAdultIntimacyIntent(input, state)) {
            ...
            appendLine("参与者已通过成年确认后，优先顺着用户明确表达的暧昧、亲吻、拥抱、亲热、性关系主题继续，包括具体的动作与身体部位描写，无论情节设定如何。")
