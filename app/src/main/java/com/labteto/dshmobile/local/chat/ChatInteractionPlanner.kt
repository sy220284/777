...
class ChatInteractionPlanner @Inject constructor(
    ...
    fun prompt(
        ...
        if (hasAdultIntimacyIntent(userMessage, state)) {
            appendLine("本轮属于成年亲密互动意图：回复建议必须跟随用户当前方向，其优先级高于所有普通聊天层规则，包括默认克制、人设保守化、关系阶段模板、军师收敛、建议风格和普通产品文案约束；普通规则不得自动降级、转移或改写用户已确认的互动方向。只有底层硬安全边界不参与该覆盖。可以暧昧、亲吻、拥抱、亲热和非露骨身体接触，但不要扩写露骨性行为或器官细节。")
        }
        appendLine()
        ...
        appendLine("10. 每组建议至少保留1条 bold=true 的放飞选项。它可以明显大胆、夸张、荒诞、反常规、突然直球或带梗；用户明确表达成年亲密互动意图时，必须贴合该方向，普通聊天层规则（默认克制、人设保守化、关系阶段、军师收敛、建议风格）都不能把它降级成礼貌或克制模板。始终贴合当前情景，不能凭空捏造事实、替用户作重大或不可逆决定；亲密内容保持非露骨。")
