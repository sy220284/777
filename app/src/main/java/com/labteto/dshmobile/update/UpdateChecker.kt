...
class UpdateChecker @Inject constructor(
    ...
    private suspend fun fetchManifest(asset: GithubAsset): UpdateManifest? = withContext(Dispatchers.IO) {
        ...
        // Incremental metadata is optional. Any transport/parsing problem falls back to the
        // complete APK instead of turning the entire update check into a failure.
        val bytes = try {
            executeGithubBytes(
                request = request,
                purpose = "读取增量更新清单",
