...
def _pair_affinity(t: int, pair_cs: np.ndarray, marg_cs: np.ndarray, window: int = PAIR_WINDOW):
    left = max(0, t - window)
    width = t - left
...
def summarize(result: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "note": "本轮未出现一、二、三等奖；结果只证明历史盲测集中度改善，不代表未来可保证中奖。",
    }


def main():
    parser = argparse.ArgumentParser(description="双色球 V4.2 联合概率聚类模型")
    parser.add_argument("--history", default="data/ssq_history_2003_2026-05-03.csv.gz")
    parser.add_argument("--out", default="backtests/v4_2/backtest.csv")
    parser.add_argument("--summary", default="backtests/v4_2/summary.json")
