...
- Chat / Work 共用同一 `AgentLoop`、模型传输、上下文预算、Session/EventLog、收件箱、取消/恢复与 Android 前台执行保活；产品能力策略保持区分：Chat 单聊只做主模型单步对话，不向模型暴露可执行工具，也不执行工具调用；Work 保留完整工具、审批、子 Agent 与工作流能力。
- 前台 Agent 收件箱已事件化：运行中追加输入使用稳定编号写入 `agent/inbox/spliced`，排队事件同时携带 transcript 与完整待执行快照；消费、恢复和取消继续写同一事实链。进程退出或切换后可从最新事件恢复待处理输入，只有 `claimed/resumed` 才进入模型历史。
...
- 主前台 Agent 的可选工具集合仍由 `LocalHarnessEngine` 持有，但严格在每个前台回合开始清空；Work 使用动态工具视图，Chat 的产品策略固定投影为空工具目录。子 Agent 与后台自动化使用独立 Agent Run 集合，当前不存在父子共享集合。
- 工具执行失败在部分 Android 适配链路中仍主要以文本返回给模型；后续应保留结构化错误、错误码和可重试信息。
- 历史压缩已从简单截尾升级为有界提取式摘要，但尚未形成显式的“目标 / 约束 / 关键决定 / 失败尝试 / 未完成事项”结构化检查点。
- 后台 Job 控制器已核心化；前台待处理用户输入已可跨进程恢复，但正在执行中的进程/代理本身仍受 Android 进程生命周期限制，尚不能在进程被杀后透明续跑。
